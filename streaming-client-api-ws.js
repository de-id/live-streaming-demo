  ...
  try {
    ...
    ws.onmessage = async (event) => {
      ...
      try {
        ...
        await sendSdpMessageWithRetry(ws, sdpMessage);
      } catch (e) {
...
async function sendSdpMessageWithRetry(ws, sdpMessage, maxRetries = 3, delay = 400) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      // Consistent delay before every attempt
      await new Promise((resolve) => setTimeout(resolve, delay));

      sendMessage(ws, sdpMessage);
      console.log(`SDP message sent successfully, attempt ${attempt}`);
      return;
    } catch (error) {
      console.error(`Failed to send SDP message, attempt ${attempt}/${maxRetries}:`, error);
      if (attempt === maxRetries) {
        throw new Error('Failed to send SDP message after max retries');
      }
    }
  }
}
